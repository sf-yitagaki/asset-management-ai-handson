...
【データソース】
  Marketplace: SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE
...
SELECT '株価（STOCK_PRICE_TIMESERIES）' AS "データ種別",
       ...
       MIN(DATE)                       AS "最古日付",
       MAX(DATE)                       AS "最新日付"
FROM SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.STOCK_PRICE_TIMESERIES
...
FROM SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.SEC_REPORT_ATTRIBUTES
...
WITH base AS (
    ...
    FROM SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.COMPANY_INDEX ci
    LEFT JOIN SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.COMPANY_CHARACTERISTICS cc
...
WITH attr AS (
    ...
    FROM SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.SEC_REPORT_ATTRIBUTES a
    JOIN SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.SEC_REPORT_INDEX i ON a.ADSH = i.ADSH
...
SELECT DATE  AS RATE_DATE,
       ...
       VALUE AS FX_RATE
FROM SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.FX_RATES_TIMESERIES
...
SELECT DATE  AS YIELD_DATE,
       '10Y' AS MATURITY,
       VALUE AS YIELD_PCT
FROM SNOWFLAKE_PUBLIC_DATA.PUBLIC_DATA_FREE.US_TREASURY_TIMESERIES
